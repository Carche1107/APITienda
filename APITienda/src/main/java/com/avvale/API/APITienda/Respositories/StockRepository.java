...
public interface StockRepository extends CrudRepository<StockModel, Long> {
    ...
    @Query(value = "SELECT id FROM stock WHERE producto_id = :product AND color_id = :color AND tienda_id = :shop", nativeQuery = true)
    Long getIdByProductoAndColorAndTienda(@Param("product") Long productId, @Param("color") Long colorId, @Param("shop") Long shopId);
